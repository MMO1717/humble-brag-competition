...
def choose_strategy(
    ...
    """Choose response_strategy from general social context, not dev-case rules."""
    ...
    if goal in {"be_supportive", "be_supportive_without_overpraising"}:
        ...
        if platform == "direct_message" and relationship == "close_friend" and mechanism in {"understated_flex", "faux_modesty", "achievement_drop", "scarcity_flex"}:
            return "validate"
        if restrained:
            return "neutral_observation"
        return "light_acknowledgment"
