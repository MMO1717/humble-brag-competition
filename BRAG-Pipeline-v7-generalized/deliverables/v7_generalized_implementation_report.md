# BRAG-Pipeline v7 generalized 实施报告

## 摘要

本次在独立工作副本 `/Users/mm/Desktop/BRAG-Pipeline-v7-generalized` 中实现了 `v7 generalized` 方向。由于当前环境无法解析 `github.com`，没有从远端真实 clone，也没有 push；工作副本已配置目标 remote，网络恢复后可迁移到 `fflow2023/BRAG-Pipeline` 的 `codex/v7-local-qwen-generalized` 分支。
...
| `src/postprocess.py` | 将 fallback response 改成 v6e 风格 generalized template pool | 提高回复多样性，减少固定模板重复 |
...
| stress format checker | 0 errors / 0 warnings |
...
| 阻塞 | 状态 | 后续动作 |
|---|---|---|
| GitHub | `git ls-remote` 报 `Could not resolve host: github.com` | 网络恢复后重新 clone 远端或将本目录变更迁移进真实 clone |
| Ollama | `127.0.0.1:11434` 连接失败 | 启动 Ollama 后先跑 `curl /api/tags` 和 `curl /v1/models` |
| qwen14b | 当前无法确认模型名 | 以 `/api/tags` 返回的实际模型名配置 `.env` |

## 下一步

1. 网络恢复后，从 `https://github.com/fflow2023/BRAG-Pipeline.git` 正式 clone。
2. 将本工作副本中的 v7 generalized 改动迁移到 `codex/v7-local-qwen-generalized`。
3. Ollama 恢复后先跑 `MAX_ITEMS=3` smoke test。
4. 再跑完整 dev 45 条，生成 `RES.md`、`format_report.json`、`dev_eval_report.json`。
5. 如果 qwen14b 可用，用 qwen14b 重跑同样流程；否则先保留 qwen3 baseline。
