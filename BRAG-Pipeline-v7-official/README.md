...
这是 CCAC 2026 赛道四 BRAG-Agent 的 v1 generalized/calibrated 架构。它会读取官方 JSONL 输入，调用本地或远程 LLM 接口，经过机制校准、策略矩阵、风险标签渲染和社会语用检查后，生成官方 7 字段 JSONL 提交文件。

当前分支只保留模型架构、运行脚本和说明文档，不内置官方数据集。使用前请把官方 `BRAG-Agent-public/` 目录放在项目根目录下，目录结构应包含：
...
```text
复制 .env.example -> 放入 BRAG-Agent-public -> 修改 config.py -> python main.py
```
...
如果使用其他 OpenAI-compatible 服务，则把 `OPENAI_BASE_URL`、`OPENAI_API_KEY` 和 `OPENAI_MODEL` 改成对应服务配置。
...
deliverables/v1_generalized_offline_check_report.md
```

## 12. v1 generalized 分支说明

`v1 generalized` 的目标不是追 public dev 最高分，而是在本地小模型 pipeline 上接入更稳健的通用社会语用模块：

- `response_strategy` 继续由 `src/strategy_rules.py` 决定，不交给模型自由生成。
- `choose_strategy()` 使用 `platform + relationship + interaction_goal + mechanism` 的通用矩阵。
- `src/postprocess.py` 的 fallback response 使用抽象 template pool 和稳定 hash，不使用具体 dev/test 实体。
- `scripts/run_no_overfit_offline_checks.py` 会扫描高风险 public-dev-specific 关键词。
- 中文实施报告保存在 `deliverables/v1_generalized_implementation_report.md`。
...
如果使用 qwen14b，`OPENAI_MODEL` 必须以 `curl http://127.0.0.1:11434/api/tags` 返回的实际模型名为准。
```
