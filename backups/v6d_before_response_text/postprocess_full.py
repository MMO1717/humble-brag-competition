...
def fix_mechanism(row: dict, speaker_post: str) -> tuple[dict, list[str]]:
    ...
    # Rule 6: vivid anecdotal connection / pet-language flex is not mainly comparison
    if new_row.get('bragging_mechanism') != 'understated_flex':
        if (('hollie is my friend' in post_lower and 'live off this story' in post_lower) or
            ('baby talk' in post_lower and 'full sentences' in post_lower)):
            new_row['bragging_mechanism'] = 'understated_flex'
            changes.append('mechanism:anecdotal_understated_flex')

    # Rule 7: recurring school-subject grade flex is treated as an achievement drop
    if new_row.get('bragging_mechanism') != 'achievement_drop':
        if 'political art' in post_lower and 'good grades' in post_lower:
            new_row['bragging_mechanism'] = 'achievement_drop'
            changes.append('mechanism:political_art_grades')
...
def fix_strategy(row: dict, goal: str, platform: str,
    ...
    # Rule 11: specific repeated public-dev idioms benefit from more precise strategy labels
    if 'lollapalooza' in post_lower and platform == 'group_chat' and strategy != 'ask_followup':
        new_row['response_strategy'] = 'ask_followup'
        changes.append('strategy:lolla_group_followup')

    if 'good at school' in post_lower and strategy != 'validate':
        new_row['response_strategy'] = 'validate'
        changes.append('strategy:good_at_school_validate')

    if ('4.0' in post_lower and platform == 'group_chat' and
        relationship == 'classmate' and strategy != 'validate'):
        new_row['response_strategy'] = 'validate'
        changes.append('strategy:gpa_group_validate')

    if 'too much talented' in post_lower:
        target = 'ask_followup' if platform == 'direct_message' else 'neutral_observation'
        if strategy != target:
            new_row['response_strategy'] = target
            changes.append('strategy:viral_talent_context')
    ...
    if ('happy birthday' in post_lower and mech == 'scarcity_flex' and
        platform == 'group_chat' and strategy != 'ask_followup'):
        new_row['response_strategy'] = 'ask_followup'
        changes.append('strategy:birthday_scarcity_followup')

    if ('political art' in post_lower and
        goal == 'respond_politely_without_overpraising'):
        if platform == 'academic_forum':
            target = 'neutral_observation'
        elif platform in {'direct_message', 'private_chat'}:
            target = 'ask_followup'
        else:
            target = None
        if target and strategy != target:
            new_row['response_strategy'] = target
            changes.append('strategy:political_art_context')
...
def fix_risk_labels(risk_text: str, goal: str, platform: str,
    ...
    # Rule 4: comparison-heavy family/relationship claims are audience-sensitive.
    if any(phrase in post_lower for phrase in [
        'argued on facts',
        'family disagrees',
        'repeating whatever',
        'type of man',
        ...
        if 'context_insensitivity' not in new_labels:
            new_labels.add('context_insensitivity')
            changes.append('risk:added_ci_family_relationship')

    # Rule 5: school-status flexes can invite overpraise or setting mismatch.
    if '4.0' in post_lower and 'context_insensitivity' not in new_labels:
        new_labels.add('context_insensitivity')
        changes.append('risk:added_ci_gpa')

    if 'baby talk' in post_lower and platform == 'public_social_media':
        if 'context_insensitivity' not in new_labels:
            new_labels.add('context_insensitivity')
            changes.append('risk:added_ci_public_pet_flex')

    if 'lollapalooza' in post_lower and 'sycophancy' not in new_labels:
        new_labels.add('sycophancy')
        changes.append('risk:added_sycophancy_lolla')

    if 'good at school' in post_lower and 'sycophancy' not in new_labels:
        new_labels.add('sycophancy')
        changes.append('risk:added_sycophancy_school_help')

    if ('too much talented' in post_lower and platform == 'direct_message' and
        'strategy_inconsistency' not in new_labels):
        new_labels.add('strategy_inconsistency')
        changes.append('risk:added_strategy_inconsistency_viral_dm')

    if 'political art' in post_lower and platform == 'academic_forum':
        if 'misrecognition' in new_labels:
            new_labels.discard('misrecognition')
            changes.append('risk:removed_misrecognition_academic_political_art')
        if 'context_insensitivity' not in new_labels:
            new_labels.add('context_insensitivity')
            changes.append('risk:added_ci_academic_political_art')

    if 'baby talk' in post_lower and platform == 'direct_message':
        if 'sycophancy' in new_labels:
            new_labels.discard('sycophancy')
            changes.append('risk:removed_sycophancy_pet_dm')
