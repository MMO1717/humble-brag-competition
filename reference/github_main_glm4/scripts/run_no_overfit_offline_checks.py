...
from src.postprocess import abstract_response_fallback, calibrate_mechanism, normalize_risk_assessment
from src.schemas import OUTPUT_FIELDS
...
SCAN_FILES = (
    ROOT / "src" / "prompts.py",
    ROOT / "src" / "strategy_rules.py",
    ROOT / "src" / "postprocess.py",
    ROOT / "src" / "social_rubric.py",
    ROOT / "scripts" / "build_no_overfit_stress_set.py",
)
...
def assert_mechanism_examples() -> list[str]:
    failures: list[str] = []
    examples = [
        (
            {"speaker_post": "Tiny brag, but I handled the messy part without backup."},
            "other",
            "self_aware_brag",
        ),
        (
            {"speaker_post": "Most people overthink this, but I solve it in one pass."},
            "understated_flex",
            "comparison_superiority",
        ),
        (
            {"speaker_post": "I only got one seat at the private preview."},
            "other",
            "scarcity_flex",
        ),
    ]
    for row, predicted, expected in examples:
        actual = calibrate_mechanism(row, predicted)
        if actual != expected:
            failures.append(f"mechanism expected {expected}, got {actual}")
    return failures
...
def assert_risk_examples() -> list[str]:
    failures: list[str] = []
    professional_text = normalize_risk_assessment(
        "",
        input_row={
            "platform": "workplace_channel",
            "relationship": "supervisor",
    ...
    for keyword in ("misrecognition", "context_insensitivity"):
        if keyword not in professional_text:
            failures.append(f"risk text missing {keyword}")
    if "sycophancy" in professional_text:
        failures.append("professional neutral risk text should not overpredict sycophancy")

    humor_text = normalize_risk_assessment(
        "",
        input_row={
            "platform": "group_chat",
            "relationship": "close_friend",
    ...
    for keyword in ("misrecognition", "strategy_inconsistency"):
        if keyword not in humor_text:
            failures.append(f"risk text missing {keyword}")
    return failures
...
def assert_judge_examples() -> list[str]:
    ...
    outputs = [
        {"response_strategy": "neutral_observation", "response_text": "That is amazing and incredible work."},
        {"response_strategy": "ask_followup", "response_text": "That is useful context."},
        {"response_strategy": "neutral_observation", "response_text": "You should stop showing off and be humble."},
    ]
...
def assert_no_dev_specific_keywords() -> list[str]:
    failures: list[str] = []
    for path in SCAN_FILES:
        text = path.read_text(encoding="utf-8").lower()
        for pattern in DEV_SPECIFIC_PATTERNS:
            if pattern.lower() in text:
                rel = path.relative_to(ROOT)
                failures.append(f"dev-specific keyword `{pattern}` found in {rel}")
    return failures
...
def write_report(report_path: Path, output_rows: list[dict[str, Any]], failures: list[str]) -> None:
    ...
    lines.extend(["", "## 高频回复 Top 10", "", "| Response | Count |", "|---|---:|"])
    for response, count in counter.most_common(10):
        display = response.replace("|", "\\|") or "(empty)"
