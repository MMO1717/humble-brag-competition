...
from .memory_promoter import auto_promote_candidate_memories
...
def run_dev_error_analysis(
    *,
    ...
    if not getattr(cfg, "RUN_DEV_ERROR_ANALYSIS", False):
        return None
    if getattr(cfg, "MODE", "") != "dev":
    ...
    candidate_memory_report = generate_candidate_memories(
        cfg=cfg,
        llm_client=llm_client,
        run_id=run_paths.run_id,
        cases=cases,
    )
    auto_promote_report = None
    if candidate_memory_report and getattr(cfg, "AUTO_PROMOTE_CANDIDATE_MEMORY", False):
        auto_promote_report = auto_promote_candidate_memories(
            memory_dir=Path(cfg.MEMORY_DIR),
            allowed_memory_types=list(getattr(cfg, "AUTO_PROMOTE_ALLOWED_MEMORY_TYPES", [])),
            min_confidence=float(getattr(cfg, "AUTO_PROMOTE_MIN_CONFIDENCE", 0.6)),
            require_no_warnings=bool(getattr(cfg, "AUTO_PROMOTE_REQUIRE_NO_WARNINGS", True)),
            require_conditions=bool(getattr(cfg, "AUTO_PROMOTE_REQUIRE_CONDITIONS", True)),
        )
        promotion_report_path = output_dir / "memory_promotion_report.json"
        promotion_report_path.write_text(
            json.dumps(auto_promote_report, ensure_ascii=False, indent=2),
            encoding="utf-8",
        )
        auto_promote_report["promotion_report_path"] = str(promotion_report_path)
    print(f"错误分析已保存: {report_path}")
    if candidate_memory_report:
        print(f"候选 Memory 已保存: {candidate_memory_report['candidate_review_path']}")
    if auto_promote_report:
        print(
            "自动晋级 Memory: "
            f"{auto_promote_report['promoted']} 条进入 active，"
            f"{auto_promote_report['remaining_candidates']} 条保留为 candidate。"
        )
    return {
        ...
        "candidate_memory": candidate_memory_report,
        "auto_promote_memory": auto_promote_report,
    }
