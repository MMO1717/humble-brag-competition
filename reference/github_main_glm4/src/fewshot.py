...
class FewShotRetriever:
    ...
    def _public_example(self, row: dict[str, Any], score: float, task: str) -> dict[str, Any]:
        """返回可放进 prompt 的示例，故意不包含 episode_id。"""
...
def build_fewshot_retriever(cfg: Any) -> FewShotRetriever | None:
    ...
    path = Path(getattr(cfg, "TRAIN_PATH"))
    try:
    ...
    return FewShotRetriever(
        train_rows=rows,
        k=getattr(cfg, "FEWSHOT_K", 2),
        min_score=getattr(cfg, "FEWSHOT_MIN_SCORE", 0.0),
        include_fields=getattr(cfg, "FEWSHOT_INCLUDE_FIELDS", True),
    )
