...
from .memory_evaluator import write_memory_eval_record
...
def print_result_summary(
    ...
    failed_episodes: dict[str, list[dict[str, Any]]] | None = None,
) -> None:
    ...
    if failed_episodes:
        print("\n⚠️  [警告] 运行期间发生异常：")
        print(f"共有 {len(failed_episodes)} 个样本在执行时触发了错误，可能导致部分或整行内容回退到默认兜底值。")
        print("请检查 debug/trace.jsonl 或下方列出的部分报错样例：")
        for ep_id, errors in list(failed_episodes.items())[:5]:
            err_msg = "; ".join(f"[{err.get('skill')}] {err.get('error')}" for err in errors)
            print(f"  - {ep_id}: {err_msg}")
        if len(failed_episodes) > 5:
            print(f"  ... 还有 {len(failed_episodes) - 5} 个样本发生报错，详细错误日志请查看 trace.jsonl")
...
def write_run_result_markdown(
    ...
    if failed_episodes:
        lines.extend([
            ...
            "## ⚠️ 运行异常警告 (Warning)",
            "> [!WARNING]",
            f"> 本次运行中有 **{len(failed_episodes)}** 个样本在执行过程中触发了异常/错误，已回退到对应字段或整行的默认兜底值。",
            "> ",
            "> **部分报错样本展示如下：**",
        ])
        for ep_id, errors in list(failed_episodes.items())[:10]:
            err_msg = "; ".join(f"`{err.get('skill')}`: {err.get('error')}" for err in errors)
            lines.append(f"> - **{ep_id}** — {err_msg}")
        if len(failed_episodes) > 10:
            lines.append(f"> - *还有 {len(failed_episodes) - 10} 个样本的报错未在此展示，请至 `debug/trace.jsonl` 查看完整记录。*")
    ...
    lines.extend(
        [
            ...
            f"- API 限速：`{manifest['enable_api_rate_limit']}`",
            f"- API RPM：`{manifest['api_requests_per_minute']}`",
            f"- API TPM：`{manifest['api_tokens_per_minute']}`",
            f"- SkillFlow：`{manifest['use_skillflow']}`",
            f"- Static memory：`{manifest['use_static_memory']}`",
            f"- Agent Memory：`{manifest['use_agent_memory']}`",
            f"- Memory mode：`{manifest['memory_mode']}`",
            f"- Active memory：`{manifest['active_memory_count']}`",
            f"- Candidate memory：`{manifest['candidate_memory_count']}`",
            f"- 本次注入 memory 次数：`{manifest['memory_used_count']}`",
            ...
            f"- LLM 调用次数：`{manifest['llm_call_count']}`",
            f"- Prompt 字符数：`{manifest['prompt_char_count']}`",
            f"- 错误分析：`{manifest['error_analysis_report_path']}`",
            "",
            "## 文件说明",
            "",
            "- `submission.jsonl`：生成的最终提交结果文件，每行一个样本输出。",
            "- `debug/trace.jsonl`：每条样本的 Agent 链路调试信息，包含机制、风险、策略、回复、模型原始输出、few-shot 示例、memory 使用情况和 skill 错误。",
            "- `debug/llm_calls.jsonl`：逐次 LLM 调用日志，包含 skill 名、prompt、原始输出、耗时和错误；仅在 `SAVE_LLM_CALL_LOGS=True` 时生成。",
            "- `../../agent_memory/eval/memory_eval_runs.jsonl`：Memory 策略实验记录，用于对比 `no_memory/static_only/active/active_plus_candidate` 等模式。",
...
def run_pipeline(cfg: ModuleType) -> list[dict[str, str]]:
    ...
    if not getattr(cfg, "USE_SKILLFLOW", True):
        raise ValueError("v2 当前以 SkillFlow 为主线，请保持 USE_SKILLFLOW = True")

    llm_client = LLMClient.from_env(
        retry_times=cfg.RETRY_TIMES,
        retry_sleep_seconds=cfg.RETRY_SLEEP_SECONDS,
        enable_rate_limit=getattr(cfg, "ENABLE_API_RATE_LIMIT", False),
        requests_per_minute=getattr(cfg, "API_REQUESTS_PER_MINUTE", None),
        tokens_per_minute=getattr(cfg, "API_TOKENS_PER_MINUTE", None),
        rate_limit_safety_margin=getattr(cfg, "API_RATE_LIMIT_SAFETY_MARGIN", 0.9),
    )
    ...
    memory_store = load_memory_store(cfg)
    wiki = memory_store.static_wiki
    if getattr(cfg, "USE_AGENT_MEMORY", False):
        memory_retriever = MemoryRetriever(
            store=memory_store,
            mode=getattr(cfg, "MEMORY_MODE", "active"),
            top_k_per_skill=getattr(cfg, "MEMORY_TOP_K_PER_SKILL", 3),
            max_chars_per_skill=getattr(cfg, "MEMORY_MAX_CHARS_PER_SKILL", 1800),
            target_skills=list(getattr(cfg, "MEMORY_TARGET_SKILLS", [])),
        )
    ...
    for index, input_row in enumerate(selected_rows, start=1):
        ...
        if state.get("skill_errors"):
            failed_episodes[episode_id] = state["skill_errors"]
    ...
    manifest = {
        ...
        "retry_times": cfg.RETRY_TIMES,
        "enable_api_rate_limit": getattr(cfg, "ENABLE_API_RATE_LIMIT", False),
        "api_requests_per_minute": getattr(cfg, "API_REQUESTS_PER_MINUTE", None),
        "api_tokens_per_minute": getattr(cfg, "API_TOKENS_PER_MINUTE", None),
        "api_rate_limit_safety_margin": getattr(cfg, "API_RATE_LIMIT_SAFETY_MARGIN", None),
        "use_skillflow": getattr(cfg, "USE_SKILLFLOW", True),
        "use_static_memory": getattr(cfg, "USE_STATIC_MEMORY", True),
        "use_agent_memory": getattr(cfg, "USE_AGENT_MEMORY", False),
        "memory_dir": str(getattr(cfg, "MEMORY_DIR", "")),
        "memory_mode": getattr(cfg, "MEMORY_MODE", "active"),
        "memory_top_k_per_skill": getattr(cfg, "MEMORY_TOP_K_PER_SKILL", None),
        "memory_target_skills": list(getattr(cfg, "MEMORY_TARGET_SKILLS", [])),
        ...
        "llm_call_count": debug_logger.llm_call_count,
        "prompt_char_count": debug_logger.prompt_char_count,
    ...
    if getattr(cfg, "USE_AGENT_MEMORY", False):
        manifest["memory_eval"] = write_memory_eval_record(
            memory_dir=Path(getattr(cfg, "MEMORY_DIR")),
            run_id=run_paths.run_id,
            manifest=manifest,
            dev_report=reports.get("evaluate_dev", {}),
            format_report=reports.get("format_checker", {}),
        )
    else:
        manifest["memory_eval"] = None
    write_json_report(run_paths.manifest_path, manifest)
    write_run_result_markdown(cfg, run_paths, reports, manifest, error_analysis_report, failed_episodes)
    print(f"本次运行清单已保存: {run_paths.manifest_path}")
    print(f"本次结果说明已保存: {run_paths.result_summary_path}")
    print_result_summary(cfg, run_paths, reports, failed_episodes)
    return output_rows
