...
SCARCITY_MECHANISM_RE = re.compile(
    r"\b(invitation-only|invite-only|private preview|closed beta|limited|restricted|"
    r"rare access|direct access|only got one seat|one seat|private session|closed group)\b",
    re.I,
)
FAUX_MODESTY_MECHANISM_RE = re.compile(
    r"\b(not sure how|somehow|i guess|apparently|i don't think i did anything special|"
    r"sorry[, ]+such|sorry for|feel blessed|feeling grateful|pretty rare|feels rare)\b",
    re.I,
)
ACHIEVEMENT_MECHANISM_RE = re.compile(
    r"\b(award|accepted|published|promotion|promoted|raise|certification|client win|"
    r"good grades|top grades|4\.0|sold|sales|finished|completed|i was there when|"
    r"became the example|became the template)\b",
    re.I,
)
HUMBLE_COMPLAINT_RE = re.compile(
    r"\b(tired|exhausted|barely rested|wish|annoying|awkward|too many|kept asking|"
    r"inbox|notifications|urgent request|more work|burden|hard to keep up)\b",
    re.I,
)
...
def calibrate_mechanism(input_row: dict[str, Any], predicted: str) -> str:
    """在模型分类后用通用 cue 做机制校准，不依赖 dev 样本 ID 或实体。"""
...
def normalize_risk_assessment(
    ...
    labels = infer_contextual_risk_labels(input_row, strategy)
