...
def _wiki_block(wiki: dict[str, str], keys: list[str], max_chars: int = 2400) -> str:
    parts = [wiki.get(key, "").strip() for key in keys if wiki.get(key, "").strip()]
    text = "\n\n".join(parts)
...
def build_understanding_prompt(
    ...
    wiki_text = _wiki_block(wiki, ["platform_relationship_style"], 1600)
    user_parts = [
    ...
    if wiki_text:
        user_parts.append(f"Relevant style notes:\n{wiki_text}")
    user_parts.append(
...
def build_risk_prompt(
    ...
    wiki_text = _wiki_block(wiki, ["risk_policy", "platform_relationship_style"], 2000)
    payload = {
        "input": {
    ...
    user_parts = [
        "Select one or two likely social response risks.",
        "Allowed risk labels: sycophancy, strategy_inconsistency, context_insensitivity, misrecognition, preachiness, over_coldness.",
        "The risk_assessment must be one short English sentence and explicitly include evaluator-detectable wording.",
        "Use these phrases in the sentence: sycophancy, strategy inconsistency, context insensitivity, misrecognition, preachiness, over-coldness.",
        f"Context:\n{json.dumps(payload, ensure_ascii=False)}",
    ]
    if wiki_text:
        user_parts.append(f"Risk notes:\n{wiki_text}")
    user_parts.append(
        "Return exactly this compact JSON shape and nothing else:\n"
        '{"risk_labels":["one_or_two_labels"],'
        '"risk_assessment":"one concise English sentence containing the selected risk keywords"}'
    )
    return [
        {
            "role": "system",
            "content": (
                "You identify concise response risks for a social reply. "
...
""".strip()
    wiki_text = _wiki_block(wiki, ["response_strategy_guide", "platform_relationship_style"], 1800)
    ...
    if wiki_text:
        user_parts.append(f"Style notes:\n{wiki_text}")
    if fewshot_text:
        user_parts.append(f"Few-shot examples:\n{fewshot_text}")
...
def build_baseline_prompt(row: dict[str, Any]) -> list[dict[str, str]]:
    user_parts = [
        "Generate a minimal BRAG-Agent prediction for this input.",
        "Return exactly one compact JSON object and nothing else.",
        "Do not include hidden reasoning, markdown, code fences, labels outside JSON, or explanations.",
        "Use only these bragging_mechanism labels: humble_complaint, faux_modesty, achievement_drop, comparison_superiority, scarcity_flex, understated_flex, self_aware_brag, other.",
        "Use only these response_strategy labels: validate, light_acknowledgment, ask_followup, humor_tease, redirect, neutral_observation, set_boundary, no_response.",
        "risk_assessment must be a short English string, not a list.",
        "response_text must be one short natural English reply and must not call out bragging.",
    ...
    return [
        {
            "role": "system",
            "content": (
                "You are a simple baseline BRAG-Agent. "
...
def mechanism_classifier_messages(row: dict[str, Any]) -> list[dict[str, str]]:
    return build_mechanism_prompt(row, {}, [])


def understanding_messages(row: dict[str, Any], mechanism: str) -> list[dict[str, str]]:
    return build_understanding_prompt(row, mechanism, {})
...
def response_messages(
    ...
    state = {
        "bragging_mechanism": mechanism,
        "speaker_intention": understanding.get("speaker_intention", ""),
        "desired_feedback": understanding.get("desired_feedback", ""),
        "risk_assessment": understanding.get("risk_assessment", ""),
        "response_strategy": strategy,
    }
    return build_response_prompt(row, state, {}, [])
