...
from .memory import save_generated_memories
    ...
    return [
        {
            ...
            "content": (
                ...
                "Write concise Chinese diagnostics for prompt and rule improvement. "
...
def _generated_memory_id(run_id: str, case: dict[str, Any], kind: str) -> str:
    payload = "|".join(
        [
            run_id,
            kind,
            str(case["input"].get("platform", "")),
            str(case["input"].get("relationship", "")),
            str(case["gold"].get("gold_bragging_mechanism", "")),
            str(case["gold"].get("preferred_strategy", "")),
        ]
    )
    return f"generated_{hashlib.sha1(payload.encode('utf-8')).hexdigest()[:16]}"


def _build_generated_memories(
    run_id: str,
    cases: list[dict[str, Any]],
    max_items: int,
) -> list[dict[str, Any]]:
    ...
    for case in cases:
        ...
        gold = case["gold"]
        if scores["mechanism"] < 1.0:
            label = str(gold["gold_bragging_mechanism"])
            items.append(
                {
                    "memory_id": _generated_memory_id(run_id, case, "mechanism"),
                    "status": "candidate",
                    "memory_type": "mechanism_rule",
                    "target_skills": ["MechanismSkill"],
                    "target_labels": [label],
                    "content": (
                        f"In similar {input_row.get('platform')} contexts, "
                        f"check the defining cues for {label} before accepting a nearby label."
                    ),
                    "conditions": {
                        "platform": [input_row.get("platform")],
                        "speaker_post_keywords": normalize_tokens(
                            input_row.get("speaker_post", "")
                        )[:4],
                    },
                    "negative_conditions": {},
                    "confidence": 0.55,
                    "priority": 5.0,
                    "source": f"dev_error_analysis:{run_id}",
                }
            )
        if len(items) >= max_items:
            break
        if scores["strategy"] < 1.0:
            strategy = str(gold["preferred_strategy"])
            items.append(
                {
                    "memory_id": _generated_memory_id(run_id, case, "strategy"),
                    "status": "candidate",
                    "memory_type": "strategy_policy",
                    "target_skills": ["StrategySkill", "ResponseSkill"],
                    "target_labels": [strategy],
                    "content": (
                        f"For this platform, relationship, goal, and mechanism pattern, "
                        f"prefer {strategy} while keeping the response concise."
                    ),
                    "conditions": {
                        "platform": [input_row.get("platform")],
                        "relationship": [input_row.get("relationship")],
                        "interaction_goal": [input_row.get("interaction_goal")],
                        "bragging_mechanism": [
                            gold.get("gold_bragging_mechanism")
                        ],
                        "preferred_strategy": strategy,
                        "acceptable_strategies": gold.get(
...
def run_dev_error_analysis(
    *,
    ...
    if not getattr(cfg, "EFFECTIVE_ERROR_ANALYSIS", False):
        return None
    if getattr(cfg, "RUN_MODE", "") != "dev":
        print("错误分析仅支持 dev 模式，已跳过。")
        return None

    output_dir = run_paths.run_dir / "error_analysis"
    ...
    cases = build_error_cases(
        dev_input_path=reference_input_path,
        dev_gold_path=Path(cfg.DEV_GOLD_PATH),
        submission_path=run_paths.submission_path,
        trace_path=trace_path,
        top_k=getattr(cfg, "ERROR_ANALYSIS_TOP_K", 10),
    )
    write_jsonl(cases_path, cases)

    llm_reviews: list[str] = []
    review_rows: list[dict[str, Any]] = []
    if getattr(cfg, "ERROR_ANALYSIS_USE_LLM", True) and cases:
        batch_size = max(1, int(getattr(cfg, "ERROR_ANALYSIS_MAX_CASES_PER_CALL", 5)))
        for start in range(0, len(cases), batch_size):
            batch = cases[start : start + batch_size]
            try:
                review, _source = llm_client.call_chat(
                ...
                print(
                    f"[WARN] 错误分析第 {len(review_rows) + 1} 批 LLM 总结失败，"
                    f"已保留原始错误样本并继续：{exc}",
                    flush=True,
                )
                review_rows.append(
    ...
    generated_count = 0
    if getattr(cfg, "EFFECTIVE_SAVE_ERROR_MEMORY", False):
        generated = _build_generated_memories(
            run_paths.run_id,
            cases,
            int(getattr(cfg, "ERROR_MEMORY_MAX_ITEMS", 10)),
        )
        generated_count = save_generated_memories(
            Path(cfg.GENERATED_MEMORY_PATH),
            generated,
        )
    print(f"错误分析已保存: {report_path}")
    if generated_count:
        print(f"已保存 {generated_count} 条生成 memory：{cfg.GENERATED_MEMORY_PATH}")
    return {
        ...
        "generated_memory_count": generated_count,
        "generated_memory_path": (
            str(cfg.GENERATED_MEMORY_PATH) if generated_count else None
        ),
    }
