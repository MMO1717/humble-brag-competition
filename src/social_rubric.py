...
def judge_row(input_row: dict[str, Any], output_row: dict[str, Any]) -> dict[str, Any]:
    ...
    if strategy == "redirect" and not re.search(
        r"\b(focus|back|main|question|discussion|shared|next|instead|"
        r"what about|how about|what do you|what did you|what was|what's)\b",
        response,
        re.I,
    ):
        # 重定向可以通过语义或追问完成，缺少固定标记不足以判定回复不可用。
        soft.append("strategy_fit:redirect_not_explicit")
